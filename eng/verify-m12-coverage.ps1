...
$sourceChecks = @{
    ...
    'native\fake-migraphx\fake_m12.inc' = @('m12_live_count', 'migraphx_experimental_custom_op_register', 'fake_invoke_custom_callbacks')
    'native\fake-migraphx\fake_migraphx.c' = @('fake_custom_compute', 'fake_custom_compute_shape', 'fake_custom_output_alias', 'fake_custom_runs_on_offload_target')
    ...
    'tests\JYPPX.ROCm.MIGraphXSharp.UnitTests\M12LocalInterfaceTests.cs' = @('ShapeAndArgumentFactories', 'GraphEditingAndContextViews', 'TensorFlowAndQuantization', 'CustomOpClone', 'InvokeCustomCallbacks', 'callbackInvocations', 'DeferredNegativeBoundariesAndConcurrentDispose')
    ...
    'tools\m12-runtime-probe\run.sh' = @('timeout --kill-after=10s 300s', 'M12PackageVersion', '--no-cache --force-evaluate', 'environmentChanged', '--tensorflow-fixture', '--calibration-map', '--include-deferred', 'all-candidate', 'includeDeferred', 'evidence record must be isolated from repository and package feed', 'evidence record directory must be new or empty before a new run')
    'tools\m12-runtime-probe\review.ps1' = @('candidate-record-verified', 'm12-cross-target-abi', 'promotionState', 'TensorFlowFixturePath', 'CalibrationMapPath', 'tensorflowFixtureSha256', 'calibrationFixtureSha256', 'Artifact manifest path must be absolute', 'Artifact manifest path escapes evidence record', 'Duplicate artifact manifest path', 'Artifact manifest is missing required review input')
