...
$latestNumericalDiagnosticPath = Join-Path $root 'compatibility\m12-provider-custom-op-numerical-diagnostic-59ee097.json'
...
if (-not (Test-Path -LiteralPath $matrixPath -PathType Leaf) -or
    ...
    -not (Test-Path -LiteralPath $latestNumericalDiagnosticPath -PathType Leaf) -or
...
$latestNumericalDiagnosticText = Get-Content -Raw -LiteralPath $latestNumericalDiagnosticPath
if (-not ($latestNumericalDiagnosticText | Test-Json -SchemaFile $numericalDiagnosticSchemaPath)) {
    throw 'Latest M12 provider custom-op numerical diagnostic does not match its JSON schema.'
}
$latestNumericalDiagnostic = $latestNumericalDiagnosticText | ConvertFrom-Json
...
if ($latestNumericalDiagnostic.sourceSha -ne '59ee097de7f2eaef15d81ade24a40a6ac62a1af1' -or
    ...
    $latestNumericalDiagnostic.numericalOutputMatched -ne $true -or
    $latestNumericalDiagnostic.promotionState -ne 'not-requested' -or
    $latestNumericalDiagnostic.officialEvidence -ne 'runtime-deferred' -or
    (@($latestNumericalDiagnostic.actualOutput) -join ',') -ne '1.25,0,3,10') {
    throw 'Latest M12 host-reference numerical diagnostic identity, result, or deferred boundary drifted.'
}
...
$sourceChecks = @{
    ...
    'compatibility\m12-provider-custom-op-numerical-diagnostic-59ee097.json' = @('59ee097de7f2eaef15d81ade24a40a6ac62a1af1', 'provider-custom-op-numerical-diagnostic', 'callbackInvocations', 'numericalOutputMatched', 'not-requested', 'runtime-deferred', 'host-reference target')
    ...
    'tests\JYPPX.ROCm.MIGraphXSharp.UnitTests\M10CapabilityEqualityTests.cs' = @('OnnxRegistryCopiesStableUtf8AndFailsClosedForEveryInjectedFault', 'SetSkipOutput', 'migraphx_get_onnx_operators_size', 'success without writing size_t output', 'SetRegistryMode(7)', 'success with empty UTF-8 string')
    ...
    'tools\m12-provider-callback-probe\Program.cs' = @('fake-native-provider-dispatch', '--provider-fixture', 'm12_runtime_provider_callback_probe', 'callback-invoked-controlled-rejection', 'callback-invoked-reference-matched', 'numerical-output', 'migraphx_shape_assign_to', 'migraphx_argument_buffer', 'migraphx_argument_create', 'migraphx_argument_assign_to', 'migraphx_argument_destroy', 'SetOutputAlias', 'OutputAliasInvocations', 'options.NumericalOutput ? "ref" : "gpu"', 'expectedInformational')
    'tools\m12-provider-callback-probe\provider-callback-probe.sh' = @('provider-callback-invocation', 'provider-custom-op-numerical-output', 'fake-native-provider-dispatch', 'promotionRequested=false', 'numericalOutputRequested', '--prebuilt-probe', '--defer-review', 'self-contained-linux-x64', 'review-deferred-to-source-host')
    'tools\m12-provider-callback-probe\review.ps1' = @('provider-callback-record-verified', 'provider-custom-op-numerical-output', 'numericalOutputMatched', 'Test-NumericVector', 'PrebuiltProbePath', 'self-contained-linux-x64', 'providerFixture', 'promotionState', 'Artifact manifest path escapes evidence record')
