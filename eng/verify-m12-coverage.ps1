...
$sourceChecks = @{
    ...
    'src\JYPPX.ROCm.MIGraphX.CSharp.API\MIGraphXExperimentalCustomOp.cs' = @('SetCompute', 'InvokeCompute', 'WriteCallbackException', '(bytes[copy] & 0xC0) == 0x80', 'Register', 'CopyState')
    'native\fake-migraphx\fake_m12.inc' = @('m12_live_count', 'migraphx_experimental_custom_op_register', 'fake_invoke_custom_callbacks', 'fake_invoke_custom_compute_with_error_buffer', 'dispatch_provider_callback', 'provider_callback_dispatch', 'callback_message')
    'native\fake-migraphx\fake_migraphx.c' = @('fake_custom_compute', 'fake_custom_compute_shape', 'fake_custom_output_alias', 'fake_custom_runs_on_offload_target', 'fake_provider_callback_message')
    ...
    'tests\JYPPX.ROCm.MIGraphXSharp.UnitTests\M12LocalInterfaceTests.cs' = @('ShapeAndArgumentFactories', 'GraphEditingAndContextViews', 'TensorFlowAndQuantization', 'CustomOpClone', 'FakeProviderDispatchInvokesRegisteredShapeCallbackThroughGraphPath', 'FakeProviderDispatchContainsShapeCallbackExceptionThroughGraphPath', 'FakeProviderDispatchIgnoresUnrelatedOperationName', 'CustomOpCallbackExceptionsBecomeNativeStatusAndUtf8Message', 'managed_utf8_exception_test', 'new UTF8Encoding(false, true)', 'InvokeCustomCallbacks', 'callbackInvocations', 'ProviderCallbackMessage', 'DeferredNegativeBoundariesAndConcurrentDispose', 'OperationAttributeSurfaceRemainsClosedOverArbitraryVariadicAbi', 'ModuleSurfaceRemainsProgramBoundWithoutIndependentOwner')
