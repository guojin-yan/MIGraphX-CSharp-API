...
$sourceChecks = @{
    ...
    'tools\m12-provider-callback-probe\provider-callback-probe.sh' = @('provider-callback-invocation', 'provider-custom-op-numerical-output', 'fake-native-provider-dispatch', 'promotionRequested=false', 'numericalOutputRequested')
    'tools\m12-provider-callback-probe\review.ps1' = @('provider-callback-record-verified', 'provider-custom-op-numerical-output', 'numericalOutputMatched', 'Test-NumericVector', 'providerFixture', 'promotionState', 'Artifact manifest path escapes evidence record')
