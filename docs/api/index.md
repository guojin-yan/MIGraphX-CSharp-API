...
The M12 batch adds graph/module/instruction views, restricted no-attribute and typed-materialized-attribute operation factories/clones, TensorFlow options and parsing, quantization option objects, experimental context access, and custom-op callback registration. `MIGraphXOperationAttributes` includes named factories for reshape, transpose, slice, multibroadcast, and topk, plus a checked generic builder for common scalar and array values. These wrappers preserve program leases and copied snapshot boundaries. Local fake-native compilation, focused tests, and the package-only candidate pass; the independent record promotes context lifetime and materialized operation attributes only, while the remaining M12 cases stay deferred. The wrappers are not included in the historical compatibility counts. Arbitrary C format placeholders and general C-varargs operation attributes remain outside the managed boundary.
