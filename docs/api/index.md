...
The M12 local batch adds graph/module/instruction views, restricted no-attribute and typed-materialized-attribute operation factories/clones, TensorFlow options and parsing, quantization option objects, experimental context access, and custom-op callback registration. These wrappers preserve program leases and copied snapshot boundaries. Local fake-native compilation and focused tests pass; real runtime validation remains deferred, and the wrappers are not included in the historical compatibility counts. Arbitrary C format placeholders and general C-varargs operation attributes remain outside the managed boundary.
