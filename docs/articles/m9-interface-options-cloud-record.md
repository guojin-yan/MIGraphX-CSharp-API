...
The prepared cloud runner deliberately reports a candidate, not a conclusion. It binds a clean pushed commit, header and ELF identity, generated model hash, public option values, GPU compile/run, output, and reference comparison into JSON. Review can promote only the observations the model actually exercises. Separate Loop and external-data fixtures, exhaustive-tune budget, and representative accuracy corpus remain visible follow-up work instead of disappearing behind one green Identity run.

中文摘要：本批次展示了“已有 P/Invoke 声明”与“可公开、可测试、可写文章的高层接口”之间仍需要参数、ownership、失败清理和证据边界设计。local substitute 与云端 official runtime 各自回答不同问题，不能相互冒充。
