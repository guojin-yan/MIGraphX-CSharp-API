...
MIGraphXSharp has an unpublished M11 `0.9.0-rc.6` local candidate while retaining `0.0.0` as the repository default. M11 adds no public API: core remains 27/160, adapter 3/11, and the aggregate map 84/107/1. Final publication still requires explicit Owner authorization and closed release-readiness evidence.
...
M11 synchronizes that external promotion and freezes deterministic M4-M6 fixtures, cases, thresholds, package-only probe, cache restart, review, and Windows policy. The failed rc.5 session is diagnostic evidence only; rc.6 has no official authorization, so M11 remains `runtime-deferred` and M8 remains `release-candidate-local`. See the [M11 runtime hardening plan](validation/m11-runtime-hardening-plan.md).

中文摘要：M11 `0.9.0-rc.6` 不新增公开 API；它保留 rc.5 失败会话，并补充分阶段诊断与强制退出升级。没有新的官方授权，M11 为 `runtime-deferred`，M8 仍为 `release-candidate-local`。
