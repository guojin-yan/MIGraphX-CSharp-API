...
The M12 local batch adds construction-safe graph views (`MIGraphXModule`, `MIGraphXInstruction`, `MIGraphXInstructions`, and `MIGraphXModules`), TensorFlow parser options, quantization option objects, experimental context access, and custom-op callback registration. Graph views retain program leases, while borrowed native collections are exposed as immutable managed snapshots. These additions are source-complete and covered by local fake-native tests; real runtime validation remains deferred. Arbitrary device pointers and graph capture interop are still outside the public boundary.
