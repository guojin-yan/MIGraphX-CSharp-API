...
| Family | Target frameworks | M1/M2 evidence |
| --- | --- | --- |
...
`net7.0+` selects the generated `LibraryImport` path. Older targets select the generated `DllImport` path. The representative set executes both the M1 lifecycle and M2 restricted ONNX workflow against the local fake-native test substitute; this is not official MIGraphX runtime evidence.
