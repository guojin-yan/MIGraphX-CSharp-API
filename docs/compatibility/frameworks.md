...
| Family | Target frameworks | Managed evidence |
| --- | --- | --- |
...
The historical M10 schema 2 snapshot recorded 27 core public types and 160 non-type records. The current M12 local snapshot records 45 core public types and 308 non-type records, plus the unchanged adapter 3 types and 11 members, identically on all 15 TFMs. Representative M10 fake-native execution traverses `net46`, `netcoreapp3.1`, `net7.0`, and `net10.0`; the M12 graph fake-native path now traverses the same four representative TFMs.

`net7.0+` selects the generated `LibraryImport` path. Older targets select the generated `DllImport` path. M3 emits both 158-EntryPoint paths from one normalized model. M4 through M10 keep one public contract across all frameworks and use internal compatibility helpers instead of removing members on old TFMs. None of the local consumer or fake-native results is official MIGraphX runtime evidence.
