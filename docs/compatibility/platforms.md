...
| --- | --- | --- |
| Managed build | M1/M2/M3/M4 local validation | All 15 exact TFMs compile with the generated dual declaration paths and one M4 public contract |
| Package assets | M1/M2/M3/M4 local validation | Each TFM has a DLL and XML document in the managed-only candidate package |
| Clean consumer | M1/M2/M3/M4 local validation | Four representative consumers restore, build, and compile M4 metadata use from the candidate package |
...
中文摘要：平台表把“可编译”“完整头/官方 ELF 静态对照”“fake-native 执行”和“官方 runtime 执行”分开。M4 对象层没有新增官方 runtime 结论；精确环境的 M1/M2 已运行，Windows/macOS 仍不因候选字符串存在而获得支持声明。
