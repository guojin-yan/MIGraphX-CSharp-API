...
Local fake-native execution validates the managed ABI and behavior but is not official MIGraphX or AMD GPU evidence. The Owner deferred M1 and M2 official runtime work to one later authorized session; both stages therefore remain `runtime-deferred` until that session records real loader, parse, compile, run, and reference-output evidence at an exact pushed SHA.

中文摘要：M2 公开面仅覆盖单输入、单输出、静态且连续的 float32 ONNX 同步 offload-copy 路径。所有 owned/borrowed 生命周期都按固定实现证据处理；本地 fake 只证明托管边界，官方 MIGraphX 与 AMD GPU 证据仍统一后置。
