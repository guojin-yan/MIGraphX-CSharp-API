...
Local fake-native execution validates the managed ABI and behavior but is not official MIGraphX or AMD GPU evidence. The separate official session at `f1a11cfd1701a041cee29188f7600c85b34ae260` executed file and buffer parse, static shape inspection, `gpu` compile with offload-copy, synchronous run, and exact Identity reference comparison on one gfx1100 device. The result is bounded to the recorded environment and model.
