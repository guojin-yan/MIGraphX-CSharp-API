...
The local fake-native source now models the added ownership classes, graph views, TensorFlow options, quantization mutations, context access, custom-op callback ownership, targeted failures, and null-output creation. Its test-only callback-invoke export calls each configured custom-op delegate with opaque arguments, so `M12LocalInterfaceTests.cs` verifies callback ABI entry, clone callback rooting, registration, and cleanup without claiming provider registry behavior. The fixture was compiled with the local MSVC toolchain, all ten focused M12 tests pass on `net10.0`, and the graph path executes through both generated interop families on the representative `net46`, `netcoreapp3.1`, `net7.0`, and `net10.0` runners. The package probe additionally covers the five reviewed materialized operation-attribute factories and their clones.
...
These results are local compile and test-substitute evidence. The independent M12 records promote only two case-level decisions; they do not promote an API family, alter the historical M3--M11 maps, or establish performance, numerical accuracy, long-run, callback, ownership-negative, or cross-target behavior. The later `SetBooleanArray` addition is covered by the `f0148bc` source-bound candidate record, but that candidate remains review-required and did not request a separate promotion.
