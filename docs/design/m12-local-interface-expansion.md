...
- `MIGraphXModule`, `MIGraphXInstruction`, `MIGraphXInstructions`, `MIGraphXModules`, and the construction-restricted `MIGraphXOperation` managed graph handles. `MIGraphXOperation.Create` supports the proven no-attribute call shape plus typed, fully materialized common attributes; `Clone` creates an owned destination before native assign-to. Arbitrary C format placeholders and general varargs remain excluded. Module parameter/literal/return/allocation and instruction-with-module-args entry points retain the parent program lease.
