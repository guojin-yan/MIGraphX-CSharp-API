...
- `MIGraphXModule`, `MIGraphXInstruction`, `MIGraphXInstructions`, `MIGraphXModules`, and the construction-restricted `MIGraphXOperation` managed graph handles. `MIGraphXOperation.Create` supports the proven no-attribute call shape plus typed, fully materialized common attributes; `MIGraphXOperationAttributes.ForReshape`, `ForTranspose`, `ForSlice`, `ForMultibroadcast`, and `ForTopK` materialize the five reviewed operation forms without C varargs. The generic builder covers signed/unsigned 32-bit and 64-bit integers, finite single/double values, Boolean, string, null, and matching arrays. `Clone` creates an owned destination before native assign-to. Arbitrary C format placeholders and general varargs remain excluded. Module parameter/literal/return/allocation and instruction-with-module-args entry points retain the parent program lease.
...
- ProjectQuality tests pass all 27 tests, including the current core API snapshot of 45 types and 308 members, bilingual XML documentation, generated binding traceability, and ownership/audit checks.
