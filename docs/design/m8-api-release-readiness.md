...
The adapter package-reference workflow restores and builds through commit-scoped `artifacts/adapter-pack-obj/<version>/<full-sha>` and `artifacts/adapter-pack-restore/<version>/<full-sha>` paths (the local feed is scoped the same way). This keeps package-mode assets and same-version package caches out of the standard source-reference restore state; a subsequent source-mode build can run with `--no-restore` without requiring a repair restore.
