...
The first reviewed RID is Ubuntu 24.04 amd64 `linux-x64`, ROCm 7.2.1, and MIGraphX `2.15.0.70201-81~24.04`. `linux-x64.json` pins the HTTPS repository, AMD archive-key fingerprint and hash, `InRelease`, `Packages.gz`, and every indexed package name/version/architecture/URL/size/SHA-256. Acquisition refuses a non-AMD host, redirect, wrong architecture, metadata drift, cache hash drift, and a cache or staging path outside the repository. Before staging, the Debian outer archive and both inner tar archives are enumerated; rooted/traversing paths, escaping symbolic links, ambiguous separators/control characters, hard links, devices, sockets, and FIFOs fail closed.
