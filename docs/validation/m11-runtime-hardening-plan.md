...
The package-only probe continues to avoid GPU inventory commands. The separately invoked `run-long-run-supervisor.sh` owns the long-run observation boundary: it takes bounded read-only `rocm-smi --showmeminfo vram --showuse` snapshots immediately before and after one phase, starts continuous `amd-smi` telemetry, and defers the probe manifest until telemetry and the post-phase snapshot have exited. Every snapshot, telemetry, or probe failure fails the phase; the final manifest is written only after those outcomes are known. The supervisor has no authority to restart the host and writes `hostRestartHandledBySupervisor: false`. Before `host-async`, `verify-restart-proof.py` must validate a sanitized proof whose managed/post-boot fingerprints differ; a true managed-to-async host restart must still be performed under separate authorization and represented by that external evidence. After a phase, `verify-supervisor-record.py` checks the metadata, both snapshot exit codes, telemetry completion, and every final manifest digest without changing or promoting the record.
