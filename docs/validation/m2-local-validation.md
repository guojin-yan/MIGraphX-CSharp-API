...
| `fake-native-executed` | passed locally | File/buffer parse, shape validation, compile options, parameter map, pinned input, synchronous run, copied Identity output, frontend-missing diagnostics, injected failures, cleanup, concurrency, and fail-closed collection-count drift before and after shape/argument materialization. |
...
中文说明：M2 的 41 项 ABI、模型哈希和 fake-native 闭环已在本地通过，集合数量在索引前及快照完成后的漂移也会失败关闭；精确 SHA 的真实 MIGraphX/GPU 会话也已完成。两者证据仍分层，不扩大到动态 shape、异步或 device buffer。
