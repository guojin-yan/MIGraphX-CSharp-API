...
The first three commands need only tracked metadata. They verify deterministic SBOM/provenance, closure and size calculations, topology, state separation, path/alias rules, archive entry safety, and both controlled/direct pack rejection. Twenty-one mutation cases cover source, license, closure, architecture, traversal, alias, SBOM, promotion receipt, package marker, state, release authorization, size, exact dependency version, duplicate package paths, unresolved dependencies, missing payload licenses, archive traversal/absolute paths, symlink escape, and special devices.

`test-runtime-source.ps1` uses the Git-ignored cache populated by `prepare-runtime.ps1`. It verifies the AMD key fingerprint, signed `InRelease`, signed `Packages.gz` binding, exact root package metadata and bytes, and the outer Debian archive shape. Before any staging, both inner tar archives are enumerated and the audit rejects rooted/traversing paths, escaping symbolic links, backslash/control-character ambiguity, hard links, devices, sockets, and FIFOs. Nine source/archive mutations cover package hash/version, architecture, host, `InRelease`, traversal, absolute paths, symlink escape, and special devices. Cached `.deb`, inner archives, extracted ELF, keyring, and temporary reports are not tracked.
