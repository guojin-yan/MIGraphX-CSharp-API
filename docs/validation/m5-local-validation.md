...
The 19-test unit suite covers dynamic-dimension validation and managed/native equality, invalid or unwritten native C-bool rejection for dimensions and shape snapshots, empty-rank and multi-axis dynamic shapes, static-to-dynamic ONNX override replacement, strict UTF-8 names, dynamic collection borrowed-null/count-drift failures, file-options/load cleanup, Save/Load ownership, cache cold rebuild/warm hit, payload corruption recovery, metadata-key invalidation, same-key concurrent writers, temporary-file cleanup, and cache-key lock cleanup after use. The value-semantics regression also verifies constructor input arrays are defensively copied and exposed collection views reject mutation. A static cache override uses an empty dimension list for a scalar and requires every supplied dimension to be positive, matching the ONNX static-shape setter. Cache serialization uses a reference-counted per-key gate so high-cardinality model/option workloads do not retain one static lock object per historical key; its identity comparer follows the case sensitivity of the host filesystem. Every focused failure path returns the M5 fake-native live count to zero. `eng/verify-m5-coverage.ps1` checks the 192-item map closure (`74 supported`, `117 planned`, `1 unsupported`), 13 ownership records, and the reviewed public baseline.
