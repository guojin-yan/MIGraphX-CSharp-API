...
Argument comparison covers self, independent equal content, one-element difference, shape difference, exact status, invalid C bool, reverse concurrent calls, Dispose during a blocked comparison, and post-dispose failure. Program comparison covers empty, parsed, different input shapes, compile-state changes, independent equal structures, exact status, invalid C bool, reverse concurrency, and Dispose races.
...
中文摘要：M10 本地门禁执行 registry、argument/program comparison 的成功、失败、并发与 Dispose 边界，并在四个代表 TFM 穿过双 interop 路径。新增 3 条 core public 记录，累计映射为 84/107/1；没有新官方授权，因此阶段是 `runtime-deferred`。
