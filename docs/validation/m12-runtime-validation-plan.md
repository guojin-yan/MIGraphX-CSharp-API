...
The source/package-bound candidate record is maintained outside the source tree under `Radeon_Cloud/records/`. The package-only probe runs seven bounded shape/argument/clone/graph/operation/context cases by default and now contains four additional deferred candidate paths (TensorFlow parse, quantization, custom-op registration, and concurrent Dispose) selectable with `--case`. Negative ownership/ABI and cross-target paths remain static/deferred. The standard cloud gate separately executes the official M1 loader/object smoke and M2/M9 Identity candidate paths; those labels remain review-required.
