...
The latest unified official session revalidated M1/M2 against pushed commit `346cdd0b01a7f8039f5deb93058928403fccc7dd` while completing the M9 option smoke. The checkout was clean and detached at that SHA, and the standard script rebuilt the managed gates before official execution.
...
| .NET / PowerShell | SDK 10.0.300 / PowerShell 7.6.5 |
...
The complete cloud script exited 0 in 103 seconds. The raw M2 smoke deliberately reported `runtime-candidate-executed`. After archive transfer with matching SHA-256, a sensitive-data scan and independent JSON review asserted the commit, model hash, both parse paths, shapes, output, and reference match; only then were M2 and GPU inference recorded as `runtime-executed`.
...
中文摘要：统一验证在 `346cdd0...` 的 clean detached checkout 上重新通过。官方库、M1 生命周期、M2 file/buffer parse、gfx1100 GPU 同步编译/执行与 Identity reference 对比均已真实运行；结论只适用于表中的单一环境和受限路径。
