...
pwsh ./eng/verify-m1-abi.ps1 -HeaderPath "${header_path}" -OfficialElfPath "${resolved_library}" 2>&1 | tee "${results}/abi-exports.txt"
...
printf '{"schemaVersion":"1.0.0","commit":"%s","managedGates":"completed","header":"verified","officialNativeM1":"runtime-executed","onnx":"not-applicable-m1","gpuInference":"not-applicable-m1"}\n' "${COMMIT_SHA}" > "${results}/summary.json"
