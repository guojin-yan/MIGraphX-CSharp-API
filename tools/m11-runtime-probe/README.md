# M11 package-only runtime probe

This probe is source for a separately authorized official session. Its project has only exact package references to core/adapter `0.9.0-rc.5` and HipSharp `0.9.1`; it has no project reference or native payload.

`run.sh` requires a clean detached exact source SHA, transferred package hashes, fixed header, explicit MIGraphX/HIP paths, deterministic fixtures, a new evidence-record directory, and a local-only NuGet feed. It enforces the frozen 1,800-second session and 120-second case boundaries, runs each repeated positive case three times, and uses a second process for the cache-restart hit. Candidate JSON is always `runtime-candidate-executed-review-required`.

`review.ps1` is a separate reviewer. It recomputes package normalized identities, exact case sets, result/artifact hashes, registry stability, source/assembly identity, exit codes, and the sensitive scan before writing an external `runtime-executed` review result. It does not review or claim long-run/timing evidence.

Do not execute either script on an official host without a new Owner authorization naming the final source/package identities, host, time window, and test layer.
