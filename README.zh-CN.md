...
- M12 与历史 M3-M11 证据映射分开记录：新增 `MIGraphXShape` scalar/stride/index 快照、argument empty/generate/load/save/clone、owned options/program assign-to clone、module/instruction 图句柄、TensorFlow options/parse、FP16/BF16/INT8/FP8 option、experimental context、raw custom-op 回调注册，以及受限的无属性和强类型已物化常用属性 operation 创建/复制；任意 C format placeholder 与通用 variadic 仍为 unsupported。详见 [M12 本地接口开发](docs/design/m12-local-interface-expansion.md)。
