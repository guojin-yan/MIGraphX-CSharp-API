...
MIGraphXSharp 现已形成未发布的 M11 `0.9.0-rc.9` 本地候选，并加入 M12 接口开发批次。M12 为剩余的 shape/argument、图编辑、TensorFlow、量化、context 和 experimental custom-op 声明增加托管 wrapper；当前本地基线为 28 项 M12 focused tests、Unit `67/67`，独立记录审核提升两个 case，其余 13 个 case 仍为 deferred。仓库默认版本仍为 `0.0.0`，没有发布任何包。
