...
- M12 与历史 M3-M11 证据映射分开记录：新增 `MIGraphXShape` scalar/stride/index 快照、argument empty/generate/load/save/clone、owned options/program assign-to clone、module/instruction 图句柄、TensorFlow options/parse、FP16/BF16/INT8/FP8 option、experimental context、raw custom-op 回调注册及受限的无属性 operation 创建/复制；通用 C 可变参数 attributes 仍为 unsupported。详见 [M12 本地接口开发](docs/design/m12-local-interface-expansion.md)。
...
- 一个 normalized model 同源生成各 158 个 `LibraryImport` 与 `DllImport` EntryPoint。通用 C 可变参数函数 `migraphx_operation_create` 仍显式标为 unsupported；托管层只暴露已验证的无属性调用形态，不猜测 attributes/variadic ABI。
