...
MIGraphXSharp `0.0.0` 现已包含 AMD 官方 MIGraphX C API 的 M1 生命周期基础、M2 受限 ONNX 闭环、M3 可复现低层绑定生产线与 M4 资源安全同步对象层。当前仍是本地工程候选，不是已发布版本。
...
请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)、[SECURITY.md](SECURITY.md)、[M3 normalized model](compatibility/m3-normalized-api.json) 和 [M4 high-level map](compatibility/m4-high-level-api-map.json)。公开 API 必须提供语义对应的中英文 XML 文档。不得提交 AMD 二进制、fake-native 构建输出、模型、凭据、云端连接信息，或无法由固定输入重现的生成声明。
