...
MIGraphXSharp 现已形成未发布的 M11 `0.9.0-rc.9` 本地候选，并加入 M12 接口开发批次。M12 为剩余的 shape/argument、图编辑、TensorFlow、量化、context 和 experimental custom-op 声明增加托管 wrapper；当前本地源码候选为 `489973a`，最终本地基线为 27 项 M12 focused tests、Unit `66/66`，独立记录审核提升两个 case，其余 13 个 case 仍为 deferred。仓库默认版本仍为 `0.0.0`，没有发布任何包。
...
- M12 与历史 M3-M11 证据映射分开记录：新增 `MIGraphXShape` scalar/stride/index 快照、argument empty/generate/load/save/clone、owned options/program assign-to clone、module/instruction 图句柄、TensorFlow options/parse、FP16/BF16/INT8/FP8 option、experimental context、raw custom-op 回调注册，以及受限的无属性和强类型已物化常用属性 operation 创建/复制；`MIGraphXOperationAttributes` 提供 reshape、transpose、slice、multibroadcast、topk 五个命名工厂及标量/数组构建器（包括 `SetBooleanArray`）；任意 C format placeholder 与通用 variadic 仍为 unsupported。布尔数组补充目前只属于本地开发，尚未纳入新的 source-bound candidate。详见 [M12 本地接口开发](docs/design/m12-local-interface-expansion.md) 和 [typed operation 示例](samples/M12TypedOperationAttributes/README.md)。
...
- 一个 normalized model 同源生成各 158 个 `LibraryImport` 与 `DllImport` EntryPoint。通用 C 可变参数函数 `migraphx_operation_create` 仍显式标为 unsupported；托管层仅暴露无属性及强类型已物化常用属性调用形态，不传递 variadic 值，也不猜测 attributes/variadic ABI。
