...
public sealed class M11RuntimeHardeningTests
{
    ...
    public void CacheIdentityChangesForNativePackageModelAndOptionsWithoutReadingGlobalState()
    {
        ...
        var baseline = new MIGraphXCacheMetadata(zero, "gpu", "offloadCopy=true", "msgpack", one, managedIdentity: "core/0.9.0-rc.3+source-a");
        var model = new MIGraphXCacheMetadata(one, "gpu", "offloadCopy=true", "msgpack", one, managedIdentity: "core/0.9.0-rc.3+source-a");
        var options = new MIGraphXCacheMetadata(zero, "gpu", "offloadCopy=false", "msgpack", one, managedIdentity: "core/0.9.0-rc.3+source-a");
        var native = new MIGraphXCacheMetadata(zero, "gpu", "offloadCopy=true", "msgpack", zero, managedIdentity: "core/0.9.0-rc.3+source-a");
        var package = new MIGraphXCacheMetadata(zero, "gpu", "offloadCopy=true", "msgpack", one, managedIdentity: "core/0.9.0-rc.3+source-b");
